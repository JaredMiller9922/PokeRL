...
def render_video(fname, all_coords, walks, bg, inter_steps=4, add_start=True):
    ...
    turbo_map = get_cmap("cet_isoluminant_cgo_80_c38").resampled(8) #mpl.colormaps['turbo']._resample(8)
    h, w = split(bg).shape[:2]
    out_path = f"{fname}.mp4"
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    wr = cv2.VideoWriter(out_path, fourcc, 30, (w, h))
    ...
    for idx in pbar:
        ...
        for fract in np.arange(0,1,1/inter_steps):
            ...
            frame = blend_overlay(bg, over)
            frame = np.ascontiguousarray(frame.astype(np.uint8))
            wr.write(cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))

            denom = sprites_rendered + len(errors)
            perc = 0 if denom == 0 else len(errors) / denom
...
def test_render(name, dat, walks, bg):
    ...
    return render_video(
        ...
        dat,
        walks,
        # TODO: Change this for really nice looking end visualizations
        # bg, inter_steps=8
        bg, inter_steps=1
    )

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Visualization Script To Show Agent Progress")

    parser.add_argument(
        "--rebuild", 
        action="store_true", 
        help="If set, rebuild the environment or cache"
    )
    args = parser.parse_args()

    # run_dir = Path('baselines/session_4da05e87_main_full') # Path('baselines/session_ebdfe818')
    run_dir = Path('../v2/runs/trajectories/vanilla_ppo') # Path('baselines/session_ebdfe818')

    # original session_e41c9eff, main session_4da05e87, extra session_e1b6d2dc
    coords_save_pth = Path('base_coords.npz')

    if (args.rebuild) and coords_save_pth.exists():
        coords_save_pth.unlink()
    ...
    procs = 8
    Path("map_vis_color").mkdir(exist_ok=True)
